...
from src.api.tool_func import exists_or_create, request, safe_dir_name
...
class ScrapTemplate(Scrap):
    def __init__(self, url: str):
        ...
        dir_path = '/'.join([
            GROUP_PATH,
            # TODO 可以自定义从url中获取文件夹名
            safe_dir_name(self.soup.find('title').text),
        ])  # 获取文件夹的绝对路径
        exists_or_create(dir_path)  # 如果不存在则创建文件夹
        Scrap.__init__(
            self,
            url=url,
            dir_path=dir_path,  # 可以以字典形式传递参数，可以登录到handle函数的kwargs参数中
        )  # 运行接口的构造函数
